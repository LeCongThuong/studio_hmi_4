...
def discover_sequence(
    ...
    for record in discover_preferred_frame_files(
        root=optimization_root,
        preferred_name=preferred_name,
        fallback_name=fallback_name,
    ):
        try:
            d = load_npy_dict(record.path)
        except Exception:
            continue
        records.append(
            FrameMeshRecord(
...
def _resize_for_display(img: np.ndarray, max_w: int, max_h: int) -> np.ndarray:
    cv2 = require_cv2("viewer image resizing")
    h, w = img.shape[:2]
...
class FrontProvider:
    def __init__(
        ...
        if self.video_path is not None:
            cv2 = require_cv2("viewer front-video playback")
    ...
    def close(self):
        if self.cap is not None:
            self.cap.release()
        require_cv2("viewer window cleanup").destroyWindow("Front View")
    ...
    def _read_front_image(self, rel_dir: str) -> Optional[np.ndarray]:
        ...
        if p is None:
            return None
        frame = require_cv2("viewer front-image loading").imread(str(p))
        return frame
    ...
    def show(
        ...
        cv2 = require_cv2("viewer front-window rendering")
...
def run_viewer(args: argparse.Namespace) -> None:
    ...
    if use_mesh:
        geom = o3d_mod.geometry.TriangleMesh()
        geom.vertices = o3d_mod.utility.Vector3dVector(vertices0.astype(np.float64))
        geom.triangles = o3d_mod.utility.Vector3iVector(faces.astype(np.int32))
        geom.compute_vertex_normals()
        vis.add_geometry(geom)
    else:
        geom = o3d_mod.geometry.PointCloud()
        geom.points = o3d_mod.utility.Vector3dVector(vertices0.astype(np.float64))
        vis.add_geometry(geom)
    ...
    def _apply_frame(i: int, force_normals: bool = False) -> None:
        ...
        if verts is None:
            return
        if use_mesh:
            geom.vertices = o3d_mod.utility.Vector3dVector(verts.astype(np.float64))
            if force_normals:
                geom.compute_vertex_normals()
        else:
            geom.points = o3d_mod.utility.Vector3dVector(verts.astype(np.float64))
